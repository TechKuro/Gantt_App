    ...
    for _ in range(len(tasks_list) * 2): # Increased loop for safety with sub-tasks
        ...
        for task in tasks_list:
            ...
            if start_date_for_task:
                ...
                # When an end_date_override is used, we must use precise fractional-day
                # calculations for laying out sub-tasks, bypassing the rounding in add_work_days.
                if end_date_override and task.get("sub_tasks"):
                    total_work_days_in_task = count_work_days(start_date_for_task, end_date_override)
                    total_defined_duration = sum(st.get('duration', 0) for st in task.get("sub_tasks"))

                    # Fallback to prevent division by zero
                    if total_defined_duration == 0:
                        total_defined_duration = 1

                    # Each unit of defined duration maps to a fraction of the total available work days
                    scale_factor = total_work_days_in_task / total_defined_duration

                    for i, sub_task in enumerate(task.get("sub_tasks", [])):
                        if i == 0:
                            sub_task["start"] = start_date_for_task
                        else:
                            # Start of this sub_task is the end of the previous one
                            sub_task["start"] = task["sub_tasks"][i-1]["end"]
                        
                        # Calculate the precise end date for this sub-task
                        scaled_duration = sub_task['duration'] * scale_factor
                        sub_task["end"] = add_work_days(sub_task["start"], scaled_duration)
                        
                else:
                    # Original logic for tasks without a fixed end date
                    for i, sub_task in enumerate(task.get("sub_tasks", [])):
...
class EditTaskDialog(simpledialog.Dialog):
    def __init__(self, parent, title, task_data, calculated_task_data, all_task_names):
    ...
    def _build_stages_ui(self):
        # Clear existing stage widgets
        for widget in self.stages_frame.winfo_children():
            if isinstance(widget, ttk.Frame) or isinstance(widget, ttk.Button): # Preserve note label
                 widget.destroy()
        ...
        for i, sub_task in enumerate(self.task_data_copy.get('sub_tasks', [])):
            ...
            if self.calculated_task_data and i < len(self.calculated_task_data.get('sub_tasks', [])):
                calc_sub = self.calculated_task_data['sub_tasks'][i]
                if 'end' in calc_sub:
                    end_date_var.set(calc_sub['end'].strftime("%d-%m-%Y"))
        ...
        if not available_stages:
            add_stage_cb.config(state="disabled")
    ...
    def _on_task_end_date_change(self, event=None):
        if not self.calculated_task_data or not self.calculated_task_data.get('start'):
            return

        task_start_date = self.calculated_task_data['start']
        end_date_override_str = self.end_date_override_var.get()

        if not end_date_override_str: return

        try:
            end_date = datetime.strptime(end_date_override_str, "%d-%m-%Y")
        except ValueError:
            return

        if end_date < task_start_date:
            return
        
        # We need to modify the sub_tasks in our data copy
        sub_tasks_data = self.task_data_copy.get('sub_tasks', [])
        if not sub_tasks_data:
            return

        total_days = float(count_work_days(task_start_date, end_date))
        num_stages = len(sub_tasks_data)
        if num_stages == 0: return

        # Distribute total duration evenly, adding any rounding remainder to the first stage.
        ...
        for i, sub_task in enumerate(sub_tasks_data):
            stage_duration = durations[i]
            if stage_duration < 0: stage_duration = 0
            sub_task['duration'] = stage_duration
        
        # Now, rebuild the UI to reflect the data changes. This is more robust
        # than trying to update the UI variables in place.
        self._build_stages_ui()
    
    def _update_subsequent_stages(self, changed_index):
        """After a change, recalculates the dates for all following stages to 'push' them."""
        task_start_date = self.calculated_task_data.get('start')
        if not task_start_date: return

        try:
            # Find the cumulative duration up to the changed stage
            cumulative_duration = 0
            for i in range(changed_index + 1):
                cumulative_duration += self.sub_task_vars[i]['duration_var'].get()

            # Now, cascade the date changes from the changed stage onwards
            for i in range(changed_index, len(self.sub_task_vars)):
                sub_task_ui = self.sub_task_vars[i]
                current_stage_duration = sub_task_ui['duration_var'].get()

                # Calculate the start date based on the cumulative duration of all predecessors
                predecessor_duration = cumulative_duration - current_stage_duration
                current_stage_start_date = add_work_days(task_start_date, predecessor_duration)

                stage_end_date = add_work_days(current_stage_start_date, current_stage_duration)
                sub_task_ui['end_date_var'].set(stage_end_date.strftime("%d-%m-%Y"))
                
                # Update cumulative duration for the next iteration
                cumulative_duration += current_stage_duration if i != changed_index else 0

        except (tk.TclError, ValueError):
            return # Ignore if a duration entry is invalid during the process

    def _on_duration_change(self, index):
        # When a stage duration is changed manually, clear the overall task end date override
        self.end_date_override_var.set("")
        self._update_subsequent_stages(index)

    def _on_end_date_change(self, index):
        # When a stage end date is changed manually, clear the overall task end date override
        self.end_date_override_var.set("")

        sub_task_vars = self.sub_task_vars[index]
        end_date_str = sub_task_vars['end_date_var'].get()

        try:
            end_date = datetime.strptime(end_date_str, "%d-%m-%Y")
        except ValueError:
            self._update_subsequent_stages(index) # On invalid format, revert to duration-based date
            return

        # Find the start date for this stage to calculate the new duration.
        task_start_date = self.calculated_task_data.get('start')
        if not task_start_date: return
        
        # Calculate the cumulative duration of all stages before this one
        ...
        if end_date < start_date:
            self._update_subsequent_stages(index) # Revert if end date is before start date
            return

        duration = count_work_days(start_date, end_date)
        sub_task_vars['duration_var'].set(float(duration))
        
        # Now that duration is updated, push subsequent stages
        self._update_subsequent_stages(index)
...
class GanttChartApp(tk.Tk):
    ...
    def edit_task(self, item_id=None):
        ...
        if task_to_edit:
            ...
            dialog = EditTaskDialog(self, "Edit Task", task_to_edit, calculated_task_data, other_task_names)
